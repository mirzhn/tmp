...
;with sq1 as (
		select 
			...
			(select coalesce(sum(KviQuantity), 0) from LostSalesDayGroups where LocationId = l.Id and [Date] = @analyzeDate) as ForecastDay,
			(select coalesce(sum(KviQuantity), 0) from LostSalesDayGroups where LocationId = l.Id and [Date] = dateadd(day, -1, @analyzeDate)) as Forecast1DayAgo,
			(select coalesce(sum(KviQuantity), 0) from LostSalesDayGroups where LocationId = l.Id and [Date] = dateadd(day, -2, @analyzeDate)) as Forecast2DayAgo,
			(select coalesce(sum(KviQuantity), 0) from LostSalesDayGroups where LocationId = l.Id and [Date] = dateadd(day, -3, @analyzeDate)) as Forecast3DayAgo,
			(select coalesce(sum(KviQuantity), 0) from LostSalesDayGroups where LocationId = l.Id and [Date] = dateadd(day, -4, @analyzeDate)) as Forecast4DayAgo,
			(select coalesce(sum(KviQuantity), 0) from LostSalesDayGroups where LocationId = l.Id and [Date] = dateadd(day, -5, @analyzeDate)) as Forecast5DayAgo,
			(select coalesce(sum(KviQuantity), 0) from LostSalesDayGroups where LocationId = l.Id and [Date] = dateadd(day, -6, @analyzeDate)) as Forecast6DayAgo,
			(select coalesce(sum(KviQuantity), 0) from LocationStateDayGroupSales where LocationId = l.Id and [Date] = @analyzeDate) as SalesDay,
			(select coalesce(sum(KviQuantity), 0) from LocationStateDayGroupSales where LocationId = l.Id and [Date] = dateadd(day, -1, @analyzeDate)) as Sales1DayAgo,
			(select coalesce(sum(KviQuantity), 0) from LocationStateDayGroupSales where LocationId = l.Id and [Date] = dateadd(day, -2, @analyzeDate)) as Sales2DayAgo,
			(select coalesce(sum(KviQuantity), 0) from LocationStateDayGroupSales where LocationId = l.Id and [Date] = dateadd(day, -3, @analyzeDate)) as Sales3DayAgo,
			(select coalesce(sum(KviQuantity), 0) from LocationStateDayGroupSales where LocationId = l.Id and [Date] = dateadd(day, -4, @analyzeDate)) as Sales4DayAgo,
			(select coalesce(sum(KviQuantity), 0) from LocationStateDayGroupSales where LocationId = l.Id and [Date] = dateadd(day, -5, @analyzeDate)) as Sales5DayAgo,
			(select coalesce(sum(KviQuantity), 0) from LocationStateDayGroupSales where LocationId = l.Id and [Date] = dateadd(day, -6, @analyzeDate)) as Sales6DayAgo,
			(select coalesce(sum(KviQuantity), 0) from LostSalesDayGroups where LocationId = l.Id and [Date] > dateadd(day, -7, @analyzeDate) and
																							[Date] <= @analyzeDate) as ForecastLocationWeek,
			(select coalesce(sum(KviQuantity), 0) from LocationStateDayGroupSales where LocationId = l.Id and [Date] > dateadd(day, -7, @analyzeDate) and
																						 [Date] <= @analyzeDate) as SalesLocationWeek,
			(select coalesce(sum(KviQuantity), 0) from LostSalesDayGroups where LocationId = l.Id and [Date] >= dateadd(day, -13, @analyzeDate) and
																							[Date] < dateadd(day, -6, @analyzeDate)) as ForecastLocationWeekAgo,
			(select coalesce(sum(KviQuantity), 0) from LocationStateDayGroupSales where LocationId = l.Id and [Date] >= dateadd(day, -13, @analyzeDate) and
																						 [Date] < dateadd(day, -6, @analyzeDate)) as SalesLocationWeekAgo,
			(select coalesce(sum(KviQuantity), 0) from LostSalesDayGroups where [Date] > dateadd(day, -7, @analyzeDate) and
																	  [Date] <= @analyzeDate) as ForecastWeekAllLoc,
			(select coalesce(sum(KviQuantity), 0) from LocationStateDayGroupSales where [Date] > dateadd(day, -7, @analyzeDate) and
																   [Date] <= @analyzeDate) as SalesWeekAllLoc,
			(select coalesce(sum(KviQuantity), 0) from LostSalesDayGroups where LocationId = l.Id and [Date] > dateadd(day, -28, @analyzeDate) and
																							[Date] <= @analyzeDate) as ForecastLocation4Week,
			(select coalesce(sum(KviQuantity), 0) from LocationStateDayGroupSales where LocationId = l.Id and [Date] > dateadd(day, -28, @analyzeDate) and
																 						 [Date] <= @analyzeDate) as SalesLocation4Week,
			(select coalesce(sum(KviQuantity), 0) from LostSalesDayGroups where LocationId = l.Id and [Date] >= dateadd(day, -55, @analyzeDate) and
																							[Date] < dateadd(day, -27, @analyzeDate)) as ForecastLocation4WeekAgo,
			(select coalesce(sum(KviQuantity), 0) from LocationStateDayGroupSales where LocationId = l.Id and [Date] >= dateadd(day, -55, @analyzeDate) and
																						 [Date] < dateadd(day, -27, @analyzeDate)) as SalesLocation4WeekAgo,
			(select coalesce(sum(KviQuantity), 0) from LostSalesDayGroups where [Date] > dateadd(day, -28, @analyzeDate) and
																	  [Date] <= @analyzeDate) as Forecast4WeekAllLoc,
			(select coalesce(sum(KviQuantity), 0) from LocationStateDayGroupSales where [Date] > dateadd(day, -28, @analyzeDate) and
