...
;with rec as
	...
	sq_sales as (
		select 
			...
			sum(case when ps.[Date] = @analyzeDate then ps.Quantity end) as SalesDay,	
			sum(case when ps.[Date] = dateadd(day, -1, @analyzeDate) then ps.Quantity end) as Sales1DayAgo,	
			sum(case when ps.[Date] = dateadd(day, -2, @analyzeDate) then ps.Quantity end) as Sales2DayAgo,
			sum(case when ps.[Date] = dateadd(day, -3, @analyzeDate) then ps.Quantity end) as Sales3DayAgo,
			sum(case when ps.[Date] = dateadd(day, -4, @analyzeDate) then ps.Quantity end) as Sales4DayAgo,
			sum(case when ps.[Date] = dateadd(day, -5, @analyzeDate) then ps.Quantity end) as Sales5DayAgo,
			sum(case when ps.[Date] = dateadd(day, -6, @analyzeDate) then ps.Quantity end) as Sales6DayAgo,	
			coalesce(sum(case when ps.[Date] > dateadd(day, -7, @analyzeDate) and
						           ps.[Date] <= @analyzeDate then ps.Quantity
					  else 0 end), 0) as SalesCategWeek,			
			coalesce(sum(case when ps.[Date] >= dateadd(day, -13, @analyzeDate) and
								   ps.[Date] < dateadd(day, -6, @analyzeDate) then ps.Quantity
			...
			coalesce(sum(case when ps.[Date] >= dateadd(day, -55, @analyzeDate) and
								   ps.[Date] < dateadd(day, -27, @analyzeDate) then ps.Quantity
	...
	sq_forecast as (
		select 
			...
			city.Name as CityName,
			rg.Name as RegionName,
			sum(case when sd.[Date] = @analyzeDate then sd.Quantity end) as ForecastDay,	
			sum(case when sd.[Date] = dateadd(day, -1, @analyzeDate) then sd.Quantity end) as Forecast1DayAgo,	
			sum(case when sd.[Date] = dateadd(day, -2, @analyzeDate) then sd.Quantity end) as Forecast2DayAgo,
			sum(case when sd.[Date] = dateadd(day, -3, @analyzeDate) then sd.Quantity end) as Forecast3DayAgo,
			sum(case when sd.[Date] = dateadd(day, -4, @analyzeDate) then sd.Quantity end) as Forecast4DayAgo,
			sum(case when sd.[Date] = dateadd(day, -5, @analyzeDate) then sd.Quantity end) as Forecast5DayAgo,
			sum(case when sd.[Date] = dateadd(day, -6, @analyzeDate) then sd.Quantity end) as Forecast6DayAgo,
			coalesce(sum(case when sd.[Date] > dateadd(day, -7, @analyzeDate) and
						           sd.[Date] <= @analyzeDate then sd.Quantity
					  else 0 end), 0) as ForecastCategWeek,
			
			coalesce(sum(case when sd.[Date] >= dateadd(day, -13, @analyzeDate) and
								   sd.[Date] < dateadd(day, -6, @analyzeDate) then sd.Quantity
			...
			coalesce(sum(case when sd.[Date] >= dateadd(day, -55, @analyzeDate) and
								   sd.[Date] < dateadd(day, -27, @analyzeDate) then sd.Quantity
