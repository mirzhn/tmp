...
;with
	...
	sq_sales as (
		select 
			...
			sum(case when ps.[Date] = pm.repdate then ps.KviQuantity end) as SalesDay,	
			sum(case when ps.[Date] = dateadd(day, -1, pm.repdate) then ps.KviQuantity end) as Sales1DayAgo,	
			sum(case when ps.[Date] = dateadd(day, -2, pm.repdate) then ps.KviQuantity end) as Sales2DayAgo,
			sum(case when ps.[Date] = dateadd(day, -3, pm.repdate) then ps.KviQuantity end) as Sales3DayAgo,
			sum(case when ps.[Date] = dateadd(day, -4, pm.repdate) then ps.KviQuantity end) as Sales4DayAgo,
			sum(case when ps.[Date] = dateadd(day, -5, pm.repdate) then ps.KviQuantity end) as Sales5DayAgo,
			sum(case when ps.[Date] = dateadd(day, -6, pm.repdate) then ps.KviQuantity end) as Sales6DayAgo,	
			sum(case when ps.[Date] > dateadd(day, -7, pm.repdate) and
						  ps.[Date] <= pm.repdate then ps.KviQuantity
					 else 0 end) as SalesCategWeek,			
			sum(case when ps.[Date] >= dateadd(day, -13, pm.repdate) and
						  ps.[Date] < dateadd(day, -6, pm.repdate) then ps.KviQuantity
			...
			sum(case when ps.[Date] >= dateadd(day, -55, pm.repdate) and
						  ps.[Date] < dateadd(day, -27, pm.repdate) then ps.KviQuantity
	...
	sq_forecast as (
		select 
			...
			sum(case when sd.[Date] = pm.repdate then sd.KviQuantity end) as ForecastDay,	
			sum(case when sd.[Date] = dateadd(day, -1, pm.repdate) then sd.KviQuantity end) as Forecast1DayAgo,	
			sum(case when sd.[Date] = dateadd(day, -2, pm.repdate) then sd.KviQuantity end) as Forecast2DayAgo,
			sum(case when sd.[Date] = dateadd(day, -3, pm.repdate) then sd.KviQuantity end) as Forecast3DayAgo,
			sum(case when sd.[Date] = dateadd(day, -4, pm.repdate) then sd.KviQuantity end) as Forecast4DayAgo,
			sum(case when sd.[Date] = dateadd(day, -5, pm.repdate) then sd.KviQuantity end) as Forecast5DayAgo,
			sum(case when sd.[Date] = dateadd(day, -6, pm.repdate) then sd.KviQuantity end) as Forecast6DayAgo,	
			sum(case when sd.[Date] > dateadd(day, -7, pm.repdate) and
			              sd.[Date] <= pm.repdate then sd.KviQuantity
				     else 0 end) as ForecastCategWeek,
			
			sum(case when sd.[Date] >= dateadd(day, -13, pm.repdate) and
			    		  sd.[Date] < dateadd(day, -6, pm.repdate) then sd.KviQuantity
			...
			sum(case when sd.[Date] >= dateadd(day, -55, pm.repdate) and
						  sd.[Date] < dateadd(day, -27, pm.repdate) then sd.KviQuantity
